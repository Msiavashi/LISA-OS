/*some of the instruction are not finished yet */
...
void la(char *reg, char *param)
{
	...
	while ( data_head != NULL )
	{
		if ( strcmp(data_head->label, param) == 0 )
		{
			...
			//		return;
			//case sizeof(char):
			//	regManager(reg, (int)data_head->string);   			//	return;	
			//default:
...
{	
// better to use the comment part	
	//unsigned int sum;
	int sum;
...
void lb(char *dest, char *data)
{
	...
	while ( data_head != NULL )
	{
		if ( strcmp(data_head->label, data) == 0 )
		{
			regManager(dest, data_head->byte);       
			return;
		}
		else
		{
...
{
//comment part is much closer to mips achitecture
